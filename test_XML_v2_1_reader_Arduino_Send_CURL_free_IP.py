...
def main():
    ...
    SOAPENV = ET.Element('soap:Envelope')
    REQ = ET.Element('request')
    SOAPB = ET.Element('soap:body')
    new = ET.Element('TSdata')
    Soap = ET.SubElement(SOAPENV,new)
    ...
    save_xml('whitelist.xml', new)
    print('пожалуйста подождите')
    system("curl -v -k --header 'Content-Type: text/xml;charset=UTF-8' --header 'SOAPAction:http://tempuri.org/IImporterWebService/ProcessRequest' --data @whitelist.xml http://IPADRESS/ImporterWebService")
